...
with open("tmp/"+sys.argv[2]+".nets", 'r') as f:
  ...
  for i in range(num_nets):
    ...
    for j in range(deg):
      #n, x, y = f.readline()[:-1].split()
      n = f.readline()[:-1]
      x=y=0
      net.append(n)
      xoffset.append(float(x))
      yoffset.append(float(y))
    nets.append(net)
...
area = hpwl = 0
for j, (net, netxoffsets, netyoffsets) in enumerate(zip(nets, netsxoffsets, netsyoffsets)):
  mn_x = mn_y = 1000000000000
  mx_x = mx_y = 0
  for pin, xoffset, yoffset in zip(net, netxoffsets, netyoffsets):
    done = False
    for i, m in enumerate(macros):
      if names[i] == pin:
        done = True
        ## calculate rotation here
        x = 2*m[0]+(m[2]-m[0]) + xoffset
        y = 2*m[1]+(m[3]-m[1]) + yoffset
        break
    if not done:
